...
const EditProfile = ({ user, onClose, onUpdate }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    console.log(
      "[EditProfile] Submitting form with data:",
      formData,
      "Image:",
      image
    );
    try {
      ...
      if (response) {
        ...
        onClose();
        console.log(
          "[EditProfile] Profile updated successfully, closing modal."
        );
      } else {
        console.error(
          "[EditProfile] Failed to update profile: No response object",
          response
        );
      ...
      if (error.response) {
        console.error(
          "[EditProfile] Server responded with:",
          error.response.data
        );
          ...
          {preview && (
            <div className="flex justify-center">
              <img
                src={preview}
                alt="Profile Preview"
                className="rounded-full w-24 h-24 object-cover border border-purple-400"
              />
            </div>
          )}
